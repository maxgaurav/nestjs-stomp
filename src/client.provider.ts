...
export function createClientProvider(): Provider {
  return {
    ...
    useFactory: (options: StompModuleOptions, logger: Logger) => {
      const connections = new ConnectFailover(
        options.servers,
      ...
      connections.on('connecting', connector => {
        logger.log(
          connector.serverProperties.remoteAddress.host +
            ':' +
            connector.serverProperties.remoteAddress.port,
        );
      });
      ...
      connections.on('error', (error, server) => {
        ...
        const address =
          server.serverProperties.remoteAddress.host +
          ':' +
          server.serverProperties.remoteAddress.port;
